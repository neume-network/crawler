...
// from and to are supposed to be included
async function crawl(from, to, recrawl, config, strategies, contracts, worker) {
...
function prepareNFT(contracts, log) {
...
async function fetchMetadata(nft, strategy) {
    let track = null;
    try {
...
async function updateOwnership(nft, strategy) {
    let track = undefined;
    try {
        // Get the last track
        track = (await db.getOne({
            chainId: CHAIN_ID,
            address: nft.erc721.address,
            tokenId: nft.erc721.token.id,
