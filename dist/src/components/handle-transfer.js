...
export async function handleTransfer(from, to, recrawl) {
    ...
    for (let i = from; i <= to; i += crawlStep + 1) {
        ...
        await debug(_handleTransfer, `_handleTransfer is hung up for ${this.constructor.name} ${fromBlock}-${toBlock}`).call(this, fromBlock, toBlock, recrawl);
    }
}
async function _handleTransfer(from, to, recrawl) {
    ...
    const mintNFTsPromise = [];
    const allTransferNFTs = [];
    for (let i = from; i <= to; i += getLogsBlockSpanSize + 1) {
        ...
        for (let j = 0; j < addresses.length; j += getLogsAddressSize) {
            ...
            const logs = await debug(ethGetLogs, `eth-getLogs is hung up for ${this.constructor.name} ${fromBlock}-${toBlock}-${j}`).call(this, fromBlock, toBlock, [TRANSFER_EVENT_SELECTOR], addressSlice);
            let nfts = logs.map((log) => prepareNFT(log));
            // Partition NFTs into mints and transfers
            const { mintNfts, transferNfts } = nfts.reduce((nfts, nft) => {
    ...
    await Promise.all(allTransferNFTs.map(async (nft) => {
        let alias;
        ...
        console.log("Update ownership of", nft.erc721.address, "at", nft.erc721.blockNumber, "from", nft.erc721.transaction.from, "to", nft.erc721.transaction.to);
    }));
}
function prepareNFT(log) {
