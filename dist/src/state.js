...
export async function getLastCrawledBlock(strategy) {
    ...
    const fileExists = await fs
        ...
        .catch(() => false);
    if (!fileExists)
        await saveLastCrawledBlock(strategy, createdAtBlock);
    return fs.readFile(location, "utf-8").then(parseInt);
}
export async function saveLastCrawledBlock(strategy, blockNumber) {
    const location = path.resolve(CONSTANTS.DATA_DIR, CONSTANTS.STATE.LAST_CRAWL, strategy);
    const fileExists = await fs
