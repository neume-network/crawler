...
const argv = yargs(hideBin(process.argv))
  ...
  .command(
    ...
    async (args) =>
      crawl(
        parseInt(args.argv.from),
        parseInt(args.argv.to ?? (await getLatestBlockNumber(config.rpc[0]))),
        config,
        getStrategies(strategyNames, args.argv.from, args.argv.to)
      )
  )
  .command("filter-contracts", "Find new contracts", async (args) =>
    filterContracts(
      parseInt(args.argv.from),
      parseInt(args.argv.to ?? (await getLatestBlockNumber(config.rpc[0]))),
      config,
