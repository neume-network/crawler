import { toHex, decodeLog, encodeFunctionCall, decodeParameters } from "eth-fun";

import { randomItem } from "../../utils.js";
import Lens from "./lens.js";
import { NFT } from "../../types.js";

export async function getHandle(this: Lens, profileId: number, blockNumber: number) {
...
export async function getCollectNFT(
  this: Lens,
  profileId: number,
  pubId: number,
  blockNumber: number,
  ...
  if (msg.error)
    throw new Error(
      `Error while calling getContractNFT on contract: ${JSON.stringify(msg, null, 2)}`,
    );
  ...
  if (typeof address !== "string")
    throw new Error(
      `invalid result of getContractNFT for contract: ${JSON.stringify(msg, null, 2)}`,
    );
...
export async function getHandleByAddress(this: Lens, address: string, blockNumber: number) {
  const rpcHost = randomItem(this.config.chain[this.chain].rpc);
  ...
  if (msg.error)
    throw new Error(
      `Error while calling tokenOwnerByIndex on contract: ${JSON.stringify(
        msg,
        null,
        2,
      )} \n ${address}`,
  ...
  if (typeof tokenId !== "number" || Number.isNaN(tokenId))
    throw new Error(
      `Invalid result of tokenOwnerByIndex for contract: ${JSON.stringify(msg, null, 2)}`,
    );
...
export async function getAlias(this: Lens, nft: NFT): Promise<string | null> {
  let handle = null;
